...
class modelServicio {
	...
	public function getServicio($id=0){
		$stmt = $this->mysqli->prepare("SELECT * FROM servicio WHERE idservicio = ? ; ");
	...
	public function getServicios(){
		$result = $this->mysqli->query('SELECT * FROM servicio');
		$queryId = $result->fetch_all(MYSQLI_ASSOC);
		$result->close();
		return $queryId;
	}
	...
	public function insert($obj){
		$stmt = $this->mysqli->prepare("INSERT INTO servicio(nombre) VALUES (?); ");
	...
	public function delete($id=0) {
		$stmt = $this->mysqli->prepare("DELETE FROM servicio WHERE idservicio = ? ; ");
		$stmt->bind_param('i', $id);
		$r = $stmt->execute();
		$stmt->close();
		return $r;
	...
	public function update($id,$obj) {
		if($this->checkID($id)){
			$stmt = $this->mysqli->prepare("UPDATE servicio SET nombre=? WHERE idservicio = ? ; ");
			$stmt->bind_param('si', $obj->nombre, $id);
			$r = $stmt->execute();
			$stmt->close();
			return $r;
		}
